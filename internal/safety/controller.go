...
func (c *SafetyController) Evaluate(snapshot watcher.MetricSnapshot) SafetyDecision {
	...
	if c.policy.MaxP95LatencyMs > 0 && snapshot.P95LatencyMs > c.policy.MaxP95LatencyMs {
	...
	if c.policy.MaxErrorRate > 0 && snapshot.ErrorRate > c.policy.MaxErrorRate {
		return SafetyDecision{
			ShouldStop: true,
	...
	if c.policy.MinAvailability > 0 && snapshot.Availability < c.policy.MinAvailability {
		return SafetyDecision{
			ShouldStop: true,
